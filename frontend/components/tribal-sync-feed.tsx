...
export function TribalSyncFeed() {
    ...
    const fetchPosts = async () => {
        ...
        try {
            ...
            const res = await fetch(`http://localhost:5000/api/tribe?${params}`)
    ...
    const handleLike = async (postId: string) => {
        ...
        try {
            const res = await fetch(`http://localhost:5000/api/tribe/${postId}/like`, { method: "PUT" })
            const data = await res.json()
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await fetch("http://localhost:5000/api/tribe", {
                method: "POST",
