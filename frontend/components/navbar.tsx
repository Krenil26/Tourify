        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-4">
                    ...
                    <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
                      {notifications.length > 0 ? (
                        notifications.map((notif) => (
                          <div key={notif._id} className="p-3 rounded-xl bg-foreground/[0.03] border border-foreground/[0.05] hover:border-emerald-500/20 transition-all group/notif">
                            <div className="flex gap-3">
                              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${notif.type === 'alert' ? 'bg-amber-500/10 text-amber-500' :
            ...
            <div className="flex items-center gap-4 pl-4 border-l border-foreground/10">
              <Link href="/profile" className="flex items-center gap-3 p-1.5 pr-5 rounded-full bg-foreground/[0.03] border border-foreground/[0.05] hover:bg-foreground/[0.08] hover:border-emerald-500/20 transition-all group">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white shadow-lg shadow-emerald-500/20 group-hover:scale-110 transition-transform">
                  <User className="w-4 h-4" />
                </div>
                <div className="flex flex-col">
                  <span className="text-[11px] font-bold text-foreground leading-none mb-0.5">Alex Johnson</span>
                  <span className="text-[9px] text-emerald-600 font-bold uppercase tracking-wider">Explorer</span>
                </div>
              </Link>

              <Link href="/login">
                <Button variant="ghost" size="icon" className="text-foreground/40 hover:text-destructive rounded-full transition-colors">
          ...
          {isOpen && (
            ...
            >
              <div className="py-6 border-t border-foreground/5 space-y-2 bg-background/95 backdrop-blur-2xl rounded-b-3xl shadow-2xl">
                <MobileLink href="/planner" icon={Compass} label="AI Planner" />
                <MobileLink href="/destinations" icon={Map} label="Trails" />
                <MobileLink href="/nature-guard" icon={Shield} label="Nature Guard" />
                <MobileLink href="/global-sanctuary" icon={Globe} label="Global Sanctuary" />
                <MobileLink href="/tribal-sync" icon={Users} label="Tribal Sync" />
                <MobileLink href="/wildlife-insight" icon={Bird} label="Wildlife Insight" />
                <MobileLink href="/dashboard" label="Travel Log" />

                <div className="pt-6 mt-6 border-t border-foreground/5 flex flex-col gap-3 px-4">
...
function MobileLink({ href, icon: Icon, label }: { href: string; icon?: any; label: string }) {
  return (
    <Link
      href={href}
