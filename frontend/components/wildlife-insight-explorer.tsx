...
export function WildlifeInsightExplorer() {
    ...
    const fetchWildlife = async () => {
        ...
        try {
            ...
            const [wRes, sRes] = await Promise.all([
                fetch(`http://localhost:5000/api/wildlife?${params}`),
                fetch(`http://localhost:5000/api/wildlife/stats`),
            ])
    ...
    const handleSight = async (id: string) => {
        ...
        try {
            const res = await fetch(`http://localhost:5000/api/wildlife/${id}/sight`, { method: "PUT" })
